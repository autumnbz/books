var JSResource={
	...
	researchTab : function(){
		ContentSummary.switchTab("RESEARCH");
	}
};
...
JSResource.forum_moderateMessage="Your post will be reviewed within 24 hours and posted live without notice if found legitimate, thank you for your patience."
...
JSResource.topicsPage_mobileBrowser="This page has features that do not work on mobile devices at this time. For full experience open this page on a computer with an up to date browser.<br/><br/>";
JSResource.topicsPage_browserUpdate_ie="<br/>Your current version of Internet Explorer does not support SVG which is needed to display this page.<br/><br/>You may upgrade here: <a href =\"http://windows.microsoft.com/en-US/internet-explorer/downloads/ie\" target=\"_blank\">http://windows.microsoft.com/en-US/internet-explorer/downloads/ie</a>/p>";
JSResource.topicsPage_browserUpdate_firefox="<br/>Your current version of Firefox does not support SVG which is needed to display this page.<br/><br/>You may upgrade here: <a href =\"http://www.mozilla.org/en-US/firefox/new/\" target=\"_blank\">http://www.mozilla.org/en-US/firefox/new/</a></p>";
JSResource.topicsPage_browserUpdate_opera="<br/>Your current version of Opera does not support SVG which is needed to display this page.<br/><br/>You may upgrade here: <a href =\"http://www.opera.com/download/\" target=\"_blank\">http://www.opera.com/download/</a></p>";
JSResource.topicsPage_browserUpdate_safari="<br/>Your current version of Safari does not support SVG which is needed to display this page.<br/><br/>You may upgrade here: <a href =\"http://www.apple.com/safari/download/\" target=\"_blank\">http://www.apple.com/safari/download/</a></p>";
JSResource.topicsPage_browserUpdate_chrome="<br/>Your current version of Chrome does not support SVG which is needed to display this page.<br/><br/>You may upgrade here: <a href =\"https://www.google.com/chrome/\" target=\"_blank\">https://www.google.com/chrome/</a></p>";
JSResource.topicsPage_browserUpdate_defaultMessage="<br/>This page is built with SVG and to browse our topics graph, you need to be using a <a href =\"http://en.wikipedia.org/wiki/Scalable_Vector_Graphics#Support_for_SVG_in_web_browsers\" target=\"_blank\">browser that supports SVG</a> (Firefox, IE, Safari, Chrome, etc)."+
	"<br/><br/> Mobile support is also limited at this time."+
	"<br/><br/> Thank you for your understanding.<br/><br/>";function trim(str) {
...
function disableRssLinks(){	
	$('#followUsRss').hide();
	$('#headerRss').hide();
}

function correctRssLinks(fixedToken){
	suffix="token=";	
	// check is rssUrl endWith suffix
	if(rssUrl.indexOf(suffix, rssUrl.length - suffix.length) !== -1){				
		rssUrl=rssUrl+fixedToken;		
	}
	$('#followUsBoxRss').attr('href', rssUrl);
	$('#persfeed').attr('href', rssUrl);	
	$('#followUsRss').show();
	$('#headerRss').show();
}
...
function doOnResult(text) {    
	if(text != "" && document.getElementById('rightbarcontentbox') != null) {	
	    ...
	    //create the missing entry slots and move the banners; this use case only happens in a specific scenario: we are on the last navigation page for a certain tab, there are less then 8 
	    //items found and the user clicks one of the content content items displayed --> rightbar is formed with index.jsp and less than 8 items on it
	    var startEntryNumber = -9999;	    
		for(i=1; i<9; i++){
			if (!($("#entry"+i).length)){
			    startEntryNumber = i;			   
				break;
		...
		if (startEntryNumber > 0){
			for(j=startEntryNumber; j<9; j++){
				$('#paginationRightbar').before('<div id="entry'+ j +'" class="entry" style="display:none"></div><div id="clearer'+ j +'" class="clearer"></div>');            			
			}
		}
		
		//move the content around banners if neccessary; we cannot move banner in the DOM, we must move content links around them
		//if ($(".bannerTopToMove").length){
		//	 $(".bannerTopToMove").before($("#entry1"));
		//	 $(".bannerTopToMove").before($("#clearer1"));
		//	 $(".bannerTopToMove").before($("#entry2"));
		//	 $(".bannerTopToMove").removeClass("bannerTopToMove");			 
		//}
		//if ($(".bannerBottomToMove").length){
		//	 $(".bannerBottomToMove").before($("#clearer2"));
		//	 $(".bannerBottomToMove").before($("#entry3"));
		//	 $(".bannerBottomToMove").before($("#clearer3"));
		//	 $(".bannerBottomToMove").before($("#entry4"));
		//	 $(".bannerBottomToMove").before($("#clearer4"));
		//	 $(".bannerBottomToMove").before($("#entry5"));
		//	 $(".bannerBottomToMove").before($("#clearer5"));
		//	 $(".bannerBottomToMove").before($("#entry6"));			 
		//	 $(".bannerBottomToMove").after($("#clearer8"));
		//	 $(".bannerBottomToMove").after($("#entry8"));
		//	 $(".bannerBottomToMove").after($("#clearer7"));
		//	 $(".bannerBottomToMove").after($("#entry7"));
		//	 $(".bannerBottomToMove").after($("#clearer6"));
		//	 $(".bannerBottomToMove").removeClass("bannerBottomToMove");
		//}
		...
		// hide the 2 banners
		if ($('#banner300x250_top').length){
			$('#banner300x250_top').hide();
		}
		if ($('#banner300x250_bottom').length){
...
})(jQuery);var loginWindowActive = false;
...
var newsletter={	
		saveSubscription : function(){
			var handlerFunc = function(t) {				
				var response = trim(t);
				if (response.substring(0,6)=='error:') {
					$('#errorMessage').html(response.substring(6,response.length));
		...
		validateEmail : function(){			
			...
			if($('#newsletterEmail').val().match(/^\S+@\S+\.(com|net|org|info|edu|mil|gov|biz|ws|us|tv|cc|aero|arpa|coop|int|jobs|museum|name|pro|travel|nato|.{2,2})$/gi)==null){
		...
		clearEmailText : function(){	
			var emailText =  $('#newsletterEmail').val();
			if(emailText==newsletterDefaultText){
				$('#newsletterEmail').val("");
				$('#newsletterEmail').css('color', 'black');
			}
		},
		...
		resetEmailText : function(){	
			var emailText =  $('#newsletterEmail').val();
			if(emailText.trim()==""){
				$('#newsletterEmail').val(newsletterDefaultText);
				$('#newsletterEmail').css('color', '#999999');
			}
		}		
}
var CookieManager = {
