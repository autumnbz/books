...
function getModerationMessage(type){
	var replyHTML = '<div class="comment-form" id="message_modaration">'+
	'<p class="popup-close" style="margin:5px"><a href="javascript:void(0);" onClick="javascript:hideReply'+type+'()">'+JSResource.close+'</a></p>'+
	'<p></p><div class="popup-notice">'+JSResource.forum_moderateMessage+'</div></div>';
	return replyHTML;
}
	...
	if (verifyPost()) {
		var handlerFunc = function(t) {
			...
			if (response.substring(0,6)=='error:') {		
				...
				showModerationMessageFromTooltip(currentMessage);
			}
			else {
				$('#newMessage').html(t);				
	...
	if (verifyFlatPost()) {
		var handlerFunc = function(t) {
			...
			if (response.substring(0,6)=='error:') {		
				...
				showModerationMessageFromFlat(currentFlatMessage);
			}
			else {
				$('#newMessage').html(t);
				var newMessageId = trim(document.getElementById('newMessageId').innerHTML);				
...
function showModerationMessageFromTooltip(messageId) {
	hideReplyFromFlat();
	hideReplyFromTooltip();	
	currentMessage = messageId;
	$('#replyTree_'+messageId).html(getModerationMessage("FromTooltip"));
	$('#replyTree_'+messageId).show();
}

function showModerationMessageFromFlat(messageId) {
	hideReplyFromFlat();
	hideReplyFromTooltip();
	currentFlatMessage = messageId;
	$('#replyFlat_'+messageId).html(getModerationMessage("FromFlat"));
	$('#replyFlat_'+messageId).show();
}
